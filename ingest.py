...
def sync_tool_context(store):
    """Write a compact project context summary to AI tool instruction files.

    Writes to:
    - ~/.gemini/GEMINI.md (Antigravity)
    - Project-level CLAUDE.md could also be updated, but we use the /gyrus skill instead
    """
    pages = store.get_all_pages()
    if not pages:
        print("  No project pages to sync.")
        return

    # Build a compact context block
    lines = [
        "# Project Context (auto-generated by Gyrus)",
        "",
        f"_Last synced: {datetime.now().strftime('%Y-%m-%d %H:%M')}_",
        f"_Source: ~/.gyrus/projects/ — {len(pages)} project pages_",
        "",
        "## Active Projects",
        "",
    ]
    ...
    for p in pages:
        ...
        # Get status
        status = overrides.get(slug, "unknown")
        if status == "unknown" and "## Status" in content:
            start = content.find("## Status") + len("## Status")
            end = content.find("\n##", start)
            status_line = content[start:end].strip() if end > 0 else ""
            first_word = status_line.split("|")[0].strip().split()[0].lower() if status_line else "unknown"
        ...
        if last_date:
            try:
                days_ago = (today - datetime.fromisoformat(last_date).date()).days
                if days_ago > 60 and status == "active":
                    status = "dormant"
            except (ValueError, TypeError):
                pass

        # Get overview (first paragraph after ## Overview)
        overview = ""
        if "## Overview" in content:
            start = content.find("## Overview") + len("## Overview")
            end = content.find("\n##", start)
            raw = content[start:end].strip() if end > 0 else content[start:start + 300].strip()
            # First sentence or 150 chars
            overview = raw[:150].split(". ")[0] + "." if raw else ""

        if status in ("active", "unknown"):
            active.append((slug, overview, last_date))
        elif status == "dormant":
            dormant.append((slug, overview, last_date))

    for slug, overview, last_date in sorted(active, key=lambda x: x[2] or "", reverse=True):
        lines.append(f"- **{slug}**: {overview}")

    if dormant:
        lines.append("")
        lines.append("## Dormant Projects")
        lines.append("")
        for slug, overview, _ in dormant[:5]:
            lines.append(f"- **{slug}**: {overview}")

    lines.append("")
    lines.append("_For full details: read ~/.gyrus/projects/PROJECT_NAME.md_")
    lines.append("")

    context_block = "\n".join(lines)

    # Write to tool instruction files
    targets = {}
    ...
    for label, path in targets.items():
        # Read existing content, replace or append Gyrus block
        existing = path.read_text() if path.exists() else ""
        marker_start = "# Project Context (auto-generated by Gyrus)"
        marker_end = "_For full details: read ~/.gyrus/projects/PROJECT_NAME.md_"
        ...
        if marker_start in existing:
            ...
            if end > 0:
                ...
                new_content = existing[:start] + context_block + existing[end:]
            else:
                new_content = existing[:start] + context_block
        else:
            # Append to end
            new_content = existing.rstrip() + "\n\n" + context_block if existing.strip() else context_block

        path.write_text(new_content + "\n")
        written += 1
        print(f"  ✓ {label}: {len(active)} active, {len(dormant)} dormant projects")

    if written == 0:
        print("  No AI tool instruction files found to update.")
    else:
        print(f"\n  Context synced to {written} tool(s). Your AI tools will see project context at session start.")
